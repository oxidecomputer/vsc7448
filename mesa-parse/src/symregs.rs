...
type TargetMap = HashMap::<String, (Address, HashMap::<String, Address>)>;
...
pub fn parse_symregs(s: &str) -> MemoryMap {
    let regs_within_re = Regex::new(
        r"^static const vtss_symreg_reg_t (regs_within_[A-Z0-9_]+)\[\] = \{$")
        .unwrap();
    let reggrp_within_re = Regex::new(
        r"^static const vtss_symreg_reggrp_t reggrps_within_([A-Z0-9_]+)\[\] = \{$")
        .unwrap();
    ...
    let offset_re =
        Regex::new(r"^#define VTSS_IO_ORIGIN([0-9]+)_OFFSET\s+0x([[:xdigit:]]+)$")
    ...
    for s in s.lines() {
        ...
        if let Some(caps) = reg_re.captures(s) {
            ...
            active_regs.as_mut().unwrap().1.insert(
                name.to_string(), Address { base, count, width, });
        }
        ...
        if let Some(caps) = target_re.captures(s) {
            ...
            let entry = target_list.entry(name).or_insert((caps[6].to_owned(), Vec::new()));
            assert!(entry.0 == caps[6]);
            let repl = if repl == -1 { None }
            else { Some(repl as usize) };
            entry.1.push((repl, addr + offsets.get(&caps[4]).unwrap()));
        }
    }

    println!("{:#x?}", target_list);
